...
cd database
docker-compose up -d
psql -h localhost -U postgres -d face_recognition -f init.sql
...
# 4. Download face models (if not included)
# Models will be auto-downloaded on first run
# Or manually place in: backend/app/models/buffalo_l/
...
### First API Call - Test Connection
```bash
# Health check
...
# 2. Check user (should be face_registered: false)
curl http://localhost:5000/api/users/1

# 3. Register face (use frontend to capture images)
# See section 1.1 for detailed registration process

# 4. Login with face (after registration)
# See section 2.1 for face login process
```

### Development Tips
1. **Use Swagger UI** for interactive API testing: `http://localhost:5000/api/docs`
2. **Enable Debug Mode** in development: Set `FLASK_DEBUG=True`
3. **Monitor Logs**: Check `backend/app/logs/app.log`
4. **Database GUI**: Use pgAdmin or DBeaver for PostgreSQL management
